...
import { ClaudeCodeExecutor } from './dist/codex/ClaudeCodeExecutor.js';
...
async function testCliExecutor() {
  ...
  const executor = new ClaudeCodeExecutor({
    cwd: process.cwd(),
    model: 'opus',
    timeout: 120000, // 2 minutes for test
  ...
  const codePrompt = 'Write a TypeScript function called "add" that takes two numbers and returns their sum. Output ONLY the code, no explanation.';
  console.log(`  Prompt: "${codePrompt.substring(0, 50)}..."`);

  const result2 = await executor.execute(codePrompt);
  console.log(`  Success: ${result2.success ? '✓' : '✗'}`);
  console.log(`  Output preview:\n${result2.output.substring(0, 300)}`);
  if (result2.error) {
  ...
  console.log('='.repeat(60));
  const passed = cliAvailable && result1.success && result2.success;
  console.log(`  Overall: ${passed ? '✓ PASSED' : '✗ FAILED'}`);
  console.log(`  CLI Integration: ${cliAvailable ? 'Working' : 'Not Available'}`);
  console.log(`  Simple Prompt: ${result1.success ? 'Working' : 'Failed'}`);
  console.log(`  Code Generation: ${result2.success ? 'Working' : 'Failed'}`);

  if (passed) {
    console.log('\n✓ RUBIX can now use your Max subscription via Claude Code CLI!');
    console.log('  - Simple prompts work');
    console.log('  - Code generation works');
    console.log('  - Quota detection ready');
    console.log('  - API fallback available (if ANTHROPIC_API_KEY set)');
  }
}
