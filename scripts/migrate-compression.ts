 ...
 *
 * Run: npx ts-node scripts/migrate-compression.ts
 * Or:  node --loader ts-node/esm scripts/migrate-compression.ts
 */

import { MemoryEngine } from '../src/core/MemoryEngine.js';
import { memoryCompressor } from '../src/memory/MemoryCompressor.js';
import { LLMCompressor } from '../src/memory/LLMCompressor.js';
...
async function migrate(): Promise<void> {
  ...
  for (const result of legacyEntries) {
    ...
    try {
      ...
      const decoded = memoryCompressor.decode(entry.content, memType as MemoryType);

      // 2. Re-compress with LLM
      const recompressed = await llmCompressor.compress(decoded);

      // Skip if LLM didn't actually compress
      if (recompressed.length >= decoded.length) {
        console.log(`   Skipped: LLM compression not beneficial (${decoded.length} → ${recompressed.length})`);
        skipped++;
        continue;
      }
