...
1. **Read server configuration.** Read the file `.claude/cloud-servers.json` in the god-agent directory (`D:\rubix-protocol\god-agent\.claude\cloud-servers.json`).
...
5. **Deploy to each selected server.** For each selected server:
   ...
   - Working directory must be `D:\rubix-protocol\god-agent`
   - Capture output
