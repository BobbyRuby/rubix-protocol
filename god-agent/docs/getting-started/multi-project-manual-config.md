...
```
D:\rubix-protocol\god-agent\.claude\mcp.json
```
...
```json
"cwd": "/home/user/rubix-protocol/god-agent"
```
...
```bash
# Check god-agent root exists
test -d "D:\rubix-protocol\god-agent" && echo "exists"
...
```bash
# Verify paths exist
test -d "D:\rubix-protocol\god-agent" && echo "god-agent exists"
